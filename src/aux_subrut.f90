subroutine tempot(i,j,k,dden0z,Fcal)
   ...
   adv(1)=(u_perturbed_new(i,j,k)+UU(k))*(potential_temperature_base(i+1,j,k)-potential_temperature_base(i-1,j,k))
   adv(2)=(v_perturbed_new(i,j,k)+VV(k))*(potential_temperature_base(i,j+1,k)-potential_temperature_base(i,j-1,k))
   adv(3)=w_perturbed_new(i,j,k)*(potential_temperature_base(i,j,k+1)-potential_temperature_base(i,j,k-1))

   advec=-((adv(1)+adv(2))+adv(3))

   verti=-w_perturbed_new(i,j,k)*(Tita0(k+1)-Tita0(k-1))

   calor=Fcal*Tita0(k)/(Temp0(k)*Cp)

   dtita(1)=potential_temperature_base(i+1,j,k)-potential_temperature_base(i-1,j,k)
   dtita(2)=potential_temperature_base(i,j+1,k)-potential_temperature_base(i,j-1,k)
   dtita(3)=potential_temperature_base(i,j,k+1)-potential_temperature_base(i,j,k-1)

   escal=(dtita(1)*KM1+dtita(2)*KM2)+dtita(3)*KM3

   lapla=(potential_temperature_base(i+1,j,k)+potential_temperature_base(i-1,j,k))+(potential_temperature_base(i,j+1,k)+&
      potential_temperature_base(i,j-1,k))+potential_temperature_base(i,j,k+1)+potential_temperature_base(i,j,k-1)-&
      6*potential_temperature_base(i,j,k)
   ...
   potential_temperature_new(i,j,k)=dt1*((advec+verti)/dx2+turbul+calor)+&
      potential_temperature_base(i,j,k)

   !     control de locura
   if (abs(potential_temperature_new(i,j,k)) > 30) then
      stop
   endif
...
subroutine corvap(Qvapneg)
   ...
   do concurrent(k=1:nz1)
      ...
      do concurrent(i=1:nx1, j=1:nx1)
         ...
         if (vapor_new(i,j,k)+Qvap0(k) < 0) vapor_new(i,j,k)=-Qvap0(k)
      end do
   end do
...
subroutine coraer(aerneg)
   ...
   do concurrent(k=1:nz1)
      ...
      do concurrent(i=1:nx1, j=1:nx1)
         ...
         if (aerosol_new(i,j,k)+aer0(k) < 0) aerosol_new(i,j,k)=-aer0(k)
      end do
   end do
   return
end
...
subroutine daeros(l,m,n)
   ...
   verti=-((w_perturbed_new(l,m,n+1)+w_perturbed_new(l,m,n))*(aer0(n+1)+aer0(n))-&
      (w_perturbed_new(l,m,n-1)+w_perturbed_new(l,m,n))*(aer0(n-1)+aer0(n)))/4.
...
subroutine dvapor(l,m,n)
   ...
   verti=-((w_perturbed_new(l,m,n+1)+w_perturbed_new(l,m,n))*(Qvap0(n+1)+Qvap0(n))-&
      (w_perturbed_new(l,m,n-1)+w_perturbed_new(l,m,n))*(Qvap0(n-1)+Qvap0(n)))/4.
...
subroutine inhomogeneous_velocities(i,j,k,dden0z)
   ...
   !$$
   gravitational_acceleration = G*(potential_temperature_base(i,j,k)/Tita0(k)&
...
subroutine filtro(varia1,facx,facy,facz)
   !filtro para potential_temperature_base vapor_base
   use dimensions
   use filtro01
   implicit none
   character*50 text
