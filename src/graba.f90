subroutine graba120(Den0,Temp0,Tita0,Pres00,Qvap0,cc2,aer0,UU,VV,&
   ...
   open(unit=unit_number,file=output_directory//"inis.da",status='unknown',form='unformatted')
   write(unit_number) Den0,Temp0,Tita0,Pres00,Qvap0,cc2,aer0,UU,VV
   close(unit_number)

   open(unit=unit_number,file=output_directory//"velos.da",status='unknown',form='unformatted')
   rewind unit_number
...
subroutine graba231(k, t1, W2, Titaa1, Qvap1, Qllu1, Qgra1, aer1, Qvap0, aer0, file_number)
   ...
   open(unit=unit_number,file=nombre)
   do 360 i=1,nx1
